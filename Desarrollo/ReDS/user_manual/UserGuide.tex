...
\begin{Figure}
	...
	\captionof{figure}{Rendimiento de un experimento realizado usando datos incompletos.}
...
\begin{Figure}
	...
	\captionof{figure}{Reporte cuantitativo de un experimento realizado usando datos incompletos.}
...
\begin{Figure}
	...
	\captionof{figure}{Secciones de la interfaz general del modo Reds - RD.}
	\label{fig:rd}
\end{Figure}
...
\begin{Figure}
	...
	\includegraphics[width=1\linewidth]{data-lecture-2.png}
	\captionof{figure}{Ventana de selección de dato sísmico.}
	\label{fig:data_lecture_2rd}
\end{Figure}

\begin{multicols}{2}
	
	Una vez seleccionado el dato sísmico, se debe pulsar en la opción \emph{Abrir}. Se podrá observar entonces en \circled{1} el dato sísmico cargado, como se muestra en la figura \ref{fig:data_lecture_3rd}. Aquí los datos se observan siguiendo la siguiente estructura: \circled{I} representa al directorio padre y \circled{II} es el dato sísmico cargado hijo del directorio padre.
	
	\begin{Figure}
		\centering
		\includegraphics[width=0.6\linewidth]{data-lecture-3.pdf}
		\captionof{figure}{Panel con un dato sísmico cargado.}
		\label{fig:data_lecture_3rd}
	\end{Figure}
...
El Módulo RD solo tiene un único algoritmo disponible: Fast Marching, tal como se observa en el panel \circled{2}, en el extremo izquierdo de la figura \ref{fig:rd}. El algoritmo Fast Marching no tiene parámetros a configurar, por lo que no es posible hacer una variación paramétrica.

\begin{multicols}{2}
	
	\begin{figure}[!ht]
		\centering
		\includegraphics[width=\textwidth]{blank.pdf}
		\caption{Algoritmo Fast Marching}
		\label{fig:fast}	
	\end{figure}
	
	El botón\hspace{0.5mm} \faEye \hspace{0.5mm} permite visualizar el modelo matemático del algoritmo Fast Marching.	
...
Los resultados de todos los experimentos que se realicen a través de esta aplicación se pueden observar en el panel \circled{6}, en el extremo derecho de la figura \ref{fig:rd}. En este panel se encuentran dos tipos de visualización: la visualización del rendimiento (iteraciones vs. error/psnr) y visualización de la reconstrucción, como se observa en la figura \ref{fig:main_result_1rd}.
